...
RSpec.describe Executor do
  context 'at performance' do
    ...
    executor = Executor.new(txt_file, 100)
    template = [
      40,
      456,
      654,
    ...
    it 'finds integers' do
      expect(executor.perform.sort).to eq template
    end
  end

  context 'when creating an instance of a class without argument' do
    executor = Executor.new('something', nil)

    it 'get default argument' do
      expect(executor.instance_variable_get('@argument'))
        .to eq Executor::DEFAULT_ARGUMENT
    end
  end
end
